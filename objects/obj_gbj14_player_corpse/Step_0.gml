...
if (phase == 0)
{
	if (text_timer > 0)
	{
		...
		if (text_timer <= 0)
		{
			text_id = scr_create_text_string_ext(position[0], position[1] - 32, depth - 1, "YOU DIED",
...
else if (phase == 1)
{
	...
	{
		text_id = scr_create_text_paragraph_ext(position[0], position[1] - 64, depth - 1, "Wake up\nDon Bruto!",
								1/12, global.font_lexou, fa_center, fa_middle,
								1, c_white, 3,false);
...
else if (phase == 2)
{
	if (!instance_exists(text_id))
	{
		text_id = scr_create_text_paragraph_ext(position[0], position[1] - 64, depth - 1, "You shall\nnot escape\nso easily!",
								1/12, global.font_lexou, fa_center, fa_middle,
								1, c_white, 3,false);
		text_id.text_height = 16;
		text_id.fade_rate = 1;
...
else if (phase == 3)
{
	if (!instance_exists(text_id))
	{
		text_id = scr_create_text_paragraph_ext(position[0], position[1] - 64, depth - 1, "GET BACK\nTO WORK!",
								1/12, global.font_lexou, fa_center, fa_middle,
								1, c_white, 3,false);
		text_id.text_height = 16;
		text_id.fade_rate = 1;
